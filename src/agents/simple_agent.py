...
Network commands should be standard CLI commands like 'show version', 'show interfaces', etc."""
        ...
        self.extractor = self.prompt_template | self.llm.with_structured_output(NetworkCommand)
    ...
    def process_request(self, user_input: str) -> Dict[str, str]:
        ...
        # Extract device and command using the LLM
        result: NetworkCommand = self.extractor.invoke({
            "user_input": user_input,
            "available_devices": available_devices
        })
        
        # Execute the command using Nornir
        output = self.network_manager.execute_command(result.device_name, result.command)
